...
本文是 book2skill 所用 SOP 的设计说明,解释"为什么这么做"。执行时的具体步骤请读 `SKILL.md`、`methodology/-1-*` 和 `methodology/01-*` 至 `06-*`。
...
| 来源 | 借鉴内容 |
|---|---|
| book2skill 实践经验 | 阶段 -1: 先判断一本书是否适合 skill 化,避免硬拆 |
...
3. **适配性**: 只有 full / partial fit 的书才能默认进入完整拆书;其他类型必须先改产物形态或让用户确认
4. **可验证**: 每个 skill 必须通过三重验证 + 压力测试
5. **可进化**: 每个 skill 必须附带 darwin 兼容的 test-prompts.json
6. **用户参与**: 阶段 -1 和阶段 0 之后必须让用户确认判断和骨架再继续
