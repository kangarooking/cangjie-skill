...
```
          ...
          └─────────┬─────────┘
                    ...
                    ▼
          ┌───────────────────┐
          │ 阶段 1.5: 三重验证 │  双评审 + V1 自适应 / V2 预测力 / V3 独特性
          └─────────┬─────────┘
                    │ 通过单元 + rejected/
                    ▼
          ┌───────────────────┐
          ...
          └─────────┬─────────┘
                    ...
                    ▼
          ┌───────────────────┐
          │ 阶段 4: 压力测试   │  test-prompts.json + 执行检查 + 对抗题 + 回炉
          └─────────┬─────────┘
                    │
                    ▼
          ┌───────────────────┐
...
3. **可验证**: 每个 skill 必须通过三重验证 + 执行质量检查 + 压力测试
4. **可进化**: 每个 skill 必须附带 darwin 兼容的 test-prompts.json
5. **用户参与**: 阶段 0 之后必须让用户确认骨架; 阶段 1.5 之后必须让用户确认入选名单
6. **可交付**: 流程终点是"用户能调用"— skill 必须被安装到 skills 目录,读者需求由 DIGEST.md 承接,流程质量由 SCORECARD.md 记录
