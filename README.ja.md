...
cangjie-skill は **RIA-TV++** パイプラインを使用して、本を生のテキストから構造化されたスキルのセットに変換します。現在は Stage -1 の適合判断を行ってから、6段階の蒸留に進みます：

0. **書籍適合判断** — 完全な skill pack、部分的な skill pack、代替成果物、停止のどれが適切かを判断し、`BOOK_FIT.md` を生成
...
> 結果は1つの要約ファイルではなく、`BOOK_FIT.md`、`BOOK_OVERVIEW.md`、`INDEX.md`、複数の `*/SKILL.md`、トリガー検証用の `test-prompts.json` を含む multi-skill リポジトリになります。
