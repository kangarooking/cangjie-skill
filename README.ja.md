...
このリポジトリはオープンな Agent Skills のディレクトリ構造に従っており、Codex で直接利用できます。リポジトリのディレクトリ全体を次のいずれかに配置してください：

- ユーザー単位：`$HOME/.agents/skills/cangjie-skill/`
- リポジトリ単位：`<repo>/.agents/skills/cangjie-skill/`

同梱の `agents/openai.yaml` は Codex と ChatGPT デスクトップアプリ向けの表示メタデータを提供します。Codex では `$cangjie-skill` で明示的に呼び出すか、`description` に基づく自動選択を利用できます。並行抽出は固定の同時実行数を仮定せず、現在のセッションで利用可能なサブエージェント容量に合わせて調整されます。詳細は [Codex Skills ドキュメント](https://developers.openai.com/codex/skills/) と [Codex Subagents ドキュメント](https://developers.openai.com/codex/subagents/) を参照してください。
...
2. **並行抽出** — 5つの専門エクストラクター（フレームワーク、原則、事例、反例、用語）が独立して動作し、ホストの容量が許す場合は並行またはバッチで原文から候補ユニットを抽出
...
7. **デリバリー** — 読者向けの `DIGEST.md`（本を読まずにエッセンスを読める長文ダイジェスト）を生成し、テストに合格したスキルを Claude Code、Cursor、Codex など対象ホストの skills ディレクトリにインストールして、実際に呼び出せるようにする
